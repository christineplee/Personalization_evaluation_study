...
Built from Study 1's main.py. Per-task generation flow.

Changes from Study 1:
- Schedule/BIBD -> condition assignment (NP, P0, P3, P6, P10)
- Profile submitted once, stored. Then each task generated individually.
- build_system_prompt uses all 16 items + optional content answers
"""
    ...
    with get_db() as conn:
        conn.executescript("""
            CREATE TABLE IF NOT EXISTS sessions (
                ...
                task_order TEXT, profiling_data TEXT, post_study_data TEXT, prolific_pid TEXT
            );
...
def build_profile_lines(profile_answers):
    """Format profile answers exactly as Study 1 does."""
    ...
    for q in PROFILING_QUESTIONS:
        ...
        if q["type"] == "bipolar7":
            ...
            a = q.get("anchors", ["1", "7"])
            lines.append(f'- {q["text"]} -- Response: {answer}/7 ({a[0]} to {a[1]})')
        elif q["type"] == "likert6":
            a = q.get("anchors", ["1", "6"])
            ...
            lines.append(f'- {q["text"]} -- Their response: "{answer}"')
        elif q["type"] == "ranking":
            if isinstance(answer, list):
                lines.append(f'- {q["text"]} -- Ranking: {" > ".join(str(a) for a in answer)}')
            else:
                lines.append(f'- {q["text"]} -- Response: {answer}')
    return "\n".join(lines)

def build_content_lines(task_id, content_answers):
    """Format content-specific answers."""
...
def build_system_prompt(has_profile, profile_answers, task_id, content_answers):
    ...
    # Exact Study 1 prompt wording for profile section
...
class TaskGenerateRequest(BaseModel):
    session_id: str
    task_index: int
    content_answers: dict = {}  # {cq_id: selected_option_index}
...
def create_session(request: CreateSessionRequest = CreateSessionRequest()):
    ...
    with get_db() as conn:
        conn.execute("INSERT INTO sessions (session_id, condition, created_at, task_order, prolific_pid) VALUES (?,?,?,?,?)",
                     (session_id, condition, datetime.utcnow().isoformat(), json.dumps(task_order), request.prolific_pid))
    return {"session_id": session_id, "condition": condition, "has_profile": cond["profile"],
            "num_content_qs": cond["num_content_qs"], "num_tasks": len(TASKS),
            "task_order": task_order, "tasks": [{"id": t["id"], "prompt": t["prompt"], "category": t["category"]} for t in TASKS],
...
def submit_profiling(submission: ProfilingSubmission):
    """Store profiling answers. No generation yet -- that happens per-task."""
    with get_db() as conn:
...
def generate_task(request: TaskGenerateRequest):
    """Generate ONE LLM response for a specific task. Uses stored profile + provided content answers."""
    with get_db() as conn:
    ...
    task_idx = task_order[request.task_index]
    task = TASKS[task_idx]

    profile_answers = json.loads(session["profiling_data"]) if session["profiling_data"] else {}

    # Build system prompt with profile + content answers
    ...
    # Determine which content Qs were shown
    ...
    with get_db() as conn:
        conn.execute(
            """INSERT INTO responses (session_id, task_id, task_index, content_question_ids, content_answers, system_prompt, llm_response, created_at)
               VALUES (?,?,?,?,?,?,?,?)""",
            (request.session_id, task["id"], task_idx, json.dumps(shown_cq_ids),
             json.dumps(request.content_answers), system_prompt, llm_response, datetime.utcnow().isoformat()))

    return {"task_id": task["id"], "task_index": task_idx, "category": task["category"],
